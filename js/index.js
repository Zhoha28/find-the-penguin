...
Group Members: Zhoha Damani, Damanpreet Singh Grewal, Yashkumar Patel, Vipulkumar Gupta and Neel Patel
Project Group No: 9
...
$(document).ready(function () {
  ...
  var currentScore = 0;
  var penguinClickAudio = new Audio("audio/PenguinClick.wav"); //Initialize audio file that will play when each mound is clicked
  var foundYetiAudio = new Audio("audio/FoundYeti.wav"); //Initialize audio file that will play when yeti pops up
  var gameRestartAudio = new Audio("audio/GameRestart.mp3"); //Initialize audio file that will play when the reset button is clicked/game restarts
  ...
  $(charDivs).click(function (evt) {
    ...
    if (gameEnd == false) {
      ...
      if (currentId != "yeti") { //check if the clicked div is not a yeti
      ...
      } else { //yeti has been encountered
        ...
        if (sessionStorage.highestScore < currentScore) {
          ...
          alert("Yeti ate you but you have achieved new High Score: " + currentScore); //Display an alert that Yeti ate you and display current Score which is the new High score
        } else {
